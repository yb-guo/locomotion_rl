...
def test_run_ablation_records_candidates_and_selects_best(
    ...
    monkeypatch.setattr(ablation.g1_ppo_smoke, "run_smoke", fake_run_smoke)
    args = ablation.parse_args(
        [
            "--backend",
            "cpu",
...
def test_run_ablation_records_failed_candidate_and_continues(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert "RuntimeError:reset wave" in failed[0]["blocker"]
