...
class _ActorCriticModule:
    ...
    def evaluate_raw_actions(self, observation: Any, raw_action: Any, *, active_mask: Any | None = None) -> tuple[Any, Any, Any]:
        mean, value = self(observation)
        log_std = self.log_std.expand_as(mean)
        active_mask = self.mask if active_mask is None else active_mask
        return masked_raw_gaussian_log_prob(raw_action, mean, log_std, active_mask), masked_entropy(self._normal_entropy(log_std), active_mask), value
