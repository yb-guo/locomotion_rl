"""Run task016 one-variable tilt reset ablations for G1 curriculum PPO."""
...
ABLATION_VARIANTS: tuple[AblationVariant, ...] = (
    AblationVariant(name="baseline", overrides={}),
    AblationVariant(name="lr_1e4", overrides={"lr": "0.0001"}),
    AblationVariant(
        name="termination_penalty_neg5",
        overrides={"termination_penalty": "-5.0"},
    ),
    AblationVariant(
        name="action_rate_penalty_high",
        overrides={"action_rate_penalty_scale": str(DEFAULT_ACTION_RATE_PENALTY_HIGH)},
    ),
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=curriculum.DEFAULT_UPDATES_PER_STAGE,
    )
...
def run_ablation(args: argparse.Namespace) -> dict[str, Any]:
    ...
    config = build_ablation_config(args)
    curriculum.write_json(run_dir / "config.json", config)
    ...
    summary = {
        "status": (
            "passed"
            if all(variant["status"] == "completed" for variant in variant_summaries)
            else "failed"
        ),
        "blocker": "",
        "run_dir": str(run_dir),
...
def build_ablation_config(args: argparse.Namespace) -> dict[str, Any]:
    return {
        "task": "task016-g1-long-horizon-tilt-reset-ablation",
