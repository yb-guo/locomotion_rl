"""G1 27DoF velocity-tracking MDP over a vectorized Genesis backend."""
...
G1_TASK_METRIC_NAMES = G1_REWARD_COMPONENT_NAMES + (
    "root_height",
    "height_bad",
...
class G1VelocityTrackingStep:
    ...
    def metrics(self) -> Mapping[str, Any]:
        """Neutral contract alias; ``info`` remains for legacy callers."""
...
class G1VelocityTrackingVectorizedEnv:
    ...
    def __init__(
        ...
        self.num_envs = backend.n_envs
        self.action_dim = backend.action_dim
        self.observation_dim = backend.observation_dim
        self.spec = TaskSpec(
            name="g1_27dof_velocity_tracking_genesis",
            observations={"policy": TensorSpace((self.observation_dim,))},
            action=TensorSpace((self.action_dim,), low=-1.0, high=1.0),
            max_episode_steps=self.config.max_episode_steps,
            metrics=G1_TASK_METRIC_NAMES,
            parameters={"backend": "vectorized_genesis", "robot": "unitree_g1_27dof"},
        )
    ...
    def step(self, action: Any) -> G1VelocityTrackingStep:
        ...
        final_observation = self._observation(state) if reset_count else None
        if reset_count:
