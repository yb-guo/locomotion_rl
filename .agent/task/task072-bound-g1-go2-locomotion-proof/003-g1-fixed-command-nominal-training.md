...
- 2026-08-28：R2 `E1_phase` implementation and tests completed in recovery worktree. Added the
  Task048-style phase `sin/cos` tail while keeping reward phase period `0.8 s` and E0 action/reward/PPO
  settings unchanged. Focused validation command
  `env CUDA_VISIBLE_DEVICES="" PYTHONPATH="$PWD/src"
  /home/admin1/workspace/proj/locomotion_rl/.venv/bin/python -m pytest -q
  tests/test_task072_locomotion_proof.py tests/test_whole_body_extended.py` returned `32 passed` after
  fixing checkpoint replay to rebuild 195D E1 shards. Two pre-fix E1 runs were preserved as invalidated
  artifacts under `nominal_v4/unitree_g1/E1_phase_invalidated_*`.
- 2026-08-28：R2 `E1_phase` final run completed from random initialization with seed `72072` and
  2,048,000 transitions. Artifacts under `artifacts/nominal_v4/unitree_g1/E1_phase/`: `run_manifest.json`
  SHA `4581f7bfa57fa0bbf7d7f1f9f0d3c543c7300538362acbb51ae5bcc6f6dcd4da`, `progression.json`
  SHA `eb9e11c000023b871202f1dfedfe2abd3058f5f9ce656942db29c772c8cd4cb0`, `initial.pt`
  SHA `940a4dbb5d35f57423fe9bf5ee30cb8094cf1d27f161e209c10a1018269d2770`, `final.pt`
  SHA `c13d053ea295a837516656d975ccb8bc7ca3130a90f7d397d0cba368eabb7fd2`, `walk.mp4`
  SHA `7835033f3b48e2d3049f0275f777c55fdc45cccbce6c37580037c01dbc5b09d4`, `diagnostic_rollout.json`
  SHA `dd77593dd691d2e3255eea9fa93db95a0b84dfd9983a5caf4e571c50327e635f`, `eval.json`
  SHA `5cf082aae25a38e552c8ff79815680f220faf4a986ad583472ef5d6803dde4c7`, `case_verifier.json`
  SHA `d72abdd5dc4c3e073e92ba2babe00a7b697e13d45bf2e84944f219c885a456ed`, and `e1_gate.json`
  SHA `965fe72a07ebd908223f3a5f62b824946a43b0763582d15588cba5a61d9dfc9a`.
- 2026-08-28：R2 `E1_phase` mechanistic gate passed. `e1_gate.json` verified parent `E0` config SHA
  ...
  `configuration.actor_observation_dim` and `configuration.phase_observation`. Effect report did not pass
  walking/pilot gates: 8 s diagnostic both-contact fraction `1.0`, left/right noninitial touchdowns `0/0`,
  alternating transitions `0`, forward displacement `0.1805 m`, fall_count `5`; 20 s eval selected
  zero-fall ratio `0.0`, planar error `0.3174`, yaw error `0.2648`, gravity XY `0.2121`, and visual check
  failed due persistent double-support dragging without real swing/touchdown. Per R2, phase remains accepted
  for E2 because E1 walking failure is an effect report, not the E1 mechanistic gate.
- 2026-08-28：After workstation reboot, the interrupted `E2_reward_dt` directory contained only
  `initial.pt` and `update000200.pt`; it was preserved as
  `artifacts/nominal_v4/unitree_g1/E2_reward_dt_interrupted_reboot_20260828T040811Z/` and was not
  resumed. After reviewer feedback, added the task-local `verify-e2-scale` fail-closed verifier; pre-verifier
  and verifier-fix source-drift E2 attempts were preserved as
  `E2_reward_dt_source_drift_pre_verifier_20260828T044631Z/`,
  `E2_reward_dt_source_drift_verifier_fix_20260828T050503Z/`, and
  `E2_reward_dt_source_drift_e1_legacy_default_fix_20260828T052439Z/`. One later external SIGTERM partial
  run was preserved as `E2_reward_dt_interrupted_sigterm_20260828T053401Z/`. Re-ran focused validation with
  `env CUDA_VISIBLE_DEVICES="" PYTHONPATH="$PWD/src"
  /home/admin1/workspace/proj/locomotion_rl/.venv/bin/python -m pytest -q
  tests/test_task072_locomotion_proof.py tests/test_whole_body_extended.py`, returning `36 passed`.
- 2026-08-28：R2 `E2_reward_dt` final verifier-source run completed from random initialization with
  seed `72072` and 2,048,000 transitions. The only scientific config diff from accepted `E1_phase` is
  `configuration.reward_scale`; `configuration.variant_id` is metadata. Artifacts under
  `artifacts/nominal_v4/unitree_g1/E2_reward_dt/`: `run_manifest.json` SHA
  `25ef89af5c7460701a8b8f5f2b48de876dbcf34b7405e54dac7471e261b35597`, `progression.json`
  SHA `eec0e703c75d181cb0692b9b6ec88032c8c90de24bb5ff55a8b19d744cf609ce`, `initial.pt`
  SHA `c6164581009cef60f6b1a0c4961c13c03997f64ee13db9d18feb2ea3b81e2c59`, `final.pt`
  SHA `9b47b678658540b91c378d5bf22da358a818a6f63058f23376ef3c453e6e3a3c`, `walk.mp4`
  SHA `f98567a8b5bf19d73af7ac199bd1a9e0484ca5e15e3c5be07e7d07d448f2954b`,
  `diagnostic_rollout.json` SHA `33d57053fa2e80979741bef75a037c2436800d3514042d32ae394f1baca6e51f`,
  `eval.json` SHA `e3f6ee31fa32f1d9be85bd490d5058e9f4ce422373ce8b8777b2412a06e9c6b1`,
  `case_verifier.json` SHA `8bf113e68b4ebafc1733169ba743437d94e52b63b5ad391c69792901200cf311`,
  `agent_visual_observation.json` SHA `a1e40434a48b4741138c50be510bd544b01377d726caaa097ec1f3e32ff01073`,
  and `e2_gate.json` SHA `fe41b57a91b715254f09625b73339e6403d02c1caa696ce6b73c0a8571b2910e`.
- 2026-08-28：R2 `E2_reward_dt` scale gate passed via task-local `verify-e2-scale`. `e2_gate.json`
  verified parent `E1_phase` config SHA
  `0632a74728d6b33a26747dfdb7e47f4531df3a10c82f234587bb2adc72b6c226`, reward scale `0.02`,
  phase schema still `193->195`, and first-100 updates finite. E1 parent first-100 mean value loss /
  pre-clip grad norm were `137.5181938/40.9377101`; E2 first-100 means were
  `0.02188625/5.26026373`, satisfying the `10x` value and `5x` grad thresholds and absolute caps.
  Per-sample scale integrity had `204800/204800` nonzero raw reward samples, max ratio error
  `1.862645149230957e-09`, and zero mismatch count `0`, so `E3a_adaptive_kl` is allowed. Effect
  artifacts still fail locomotion: render `render_passed=false`, diagnostic both-contact fraction `1.0`,
  left/right noninitial touchdowns `0/0`, alternating transitions `0`, forward displacement `0.3439 m`,
  fall_count `4`; 20 s eval selected zero-fall ratio `0.0`, planar error `0.2572`, yaw error `0.4468`,
  forward displacement `0.6249 m`, update0/zero displacement margins `0.2158/0.2311 m`; visual check
  failed due double-support dragging without visible swing/touchdown.
- 2026-08-28：R2 `E3a_adaptive_kl` implementation and focused tests completed. Added adaptive KL LR
  telemetry, disabled hard KL stop for E3a only, required the passed E2 `e2_gate.json` before E3a train,
  added `verify-e3-optimizer`, and bound the task-independent PPO kernel in static lineage. Focused validation command
  `env CUDA_VISIBLE_DEVICES="" PYTHONPATH="$PWD/src"
  /home/admin1/workspace/proj/locomotion_rl/.venv/bin/python -m pytest -q
  tests/test_task072_locomotion_proof.py tests/test_whole_body_extended.py` returned `41 passed`.
  The pre-lineage-fix E3a run was preserved as
  `E3a_adaptive_kl_provenance_incomplete_20260828T063858Z/`.
- 2026-08-28：R2 `E3a_adaptive_kl` provenance-complete run completed from random initialization with
  seed `72072` and 2,048,000 transitions. Artifacts under
  `artifacts/nominal_v4/unitree_g1/E3a_adaptive_kl/`: `run_manifest.json` SHA
  `3974dbadfe7d96b234878cc14fe6edfb58def64ea2a154747688bda5f118e33a`, `progression.json`
  SHA `45e9a52ee07a6166fe1ffe2cd1af43a44a4f9bda7fdb3e4f07261782db080d27`, `initial.pt`
  SHA `9d09b4d84b27e8e6a2bc88833a1812f0a15a2bda740704ca4e68d33f414cd391`, `final.pt`
  SHA `5d1afb121573175badc642992e4dc898e5eee2ee85087577a762348a6857392e`,
  `walk.mp4` SHA `e1a5fbfc16c93041b5a01237b6b31e9182ecd9e15d4b3f9b52b79de7b011ce9d`,
  `diagnostic_rollout.json` SHA `89e9a74c77143099e652b3058fc1bda4d73882ccad25e9e5b615d5dd4576aff1`,
  `eval.json` SHA `672e62f08b99828c03715c5113a9c5ddaa397280989acbead6efb0b7ee35e6e0`,
  `case_verifier.json` SHA `940f92a5071441182a3be37a0113ded65284e8927bd36f6a164170d9f28c324e`,
  `agent_visual_observation.json` SHA `d9dd064c454370c61316fef4012621bb68d383f11d5b1409f256e2011a2820dd`,
  and `e3a_gate.json` SHA `3cb5594110432cf6bc4ab10bd4d74acec470468e8fe2cb16d05177ceb1706d1e`.
- 2026-08-28：R2 `E3a_adaptive_kl` optimizer gate failed / rejected. `e3a_gate.json` verified all
  updates completed 4 epochs and 32 minibatches with hard-stop fraction `0.0`, but attempted-minibatch
  approximate KL exceeded the gate by a large margin: mean/p95/max `4.539716/3.420841/28128.240234`
  vs required `<=0.015/0.03/0.05`; clip fraction mean/p95 `0.684878/0.828125` vs required
  `<=0.20/0.35`. LR stayed within `[1e-5,1e-2]`; scheduler decisions were `increase:6, hold:994`,
  first/last LR before-after `[0.0001,0.00015] -> [0.0011390625,0.0011390625]`, scheduler KL
  mean/p95/max `0.008998/0.012922/0.018500`. Effect artifacts also failed: render
  `render_passed=false`; diagnostic both-contact fraction `0.9275`, left/right noninitial touchdowns
  `3/18`, alternating transitions `8`, fall_count `2`, forward displacement `0.3519 m`; 20 s eval
  selected zero-fall ratio `0.0`, planar error `0.3694`, yaw error `1.0213`, update0/zero displacement
  margins `0.0744/0.0897 m`; visual check observed small rocking/leaning motion without reliable
  alternating swing, single-support, or touchdown cycle. Per user gate-fail stop rule, no E4 or E3b run
  was started in this pass.
- 2026-08-28：只读 root-cause audit 对照本机 `mjlab==1.2.0` / `rsl-rl-lib==5.0.1`，确认当前 E3a
  只复用了 adaptive LR 阈值/倍率，并未复现其关键语义：Task072 scheduler KL 对 batch×29 active
  elements 取均值而 joint policy log-prob 求和，scheduler 每 update 末才决策一次；高方差下又因只
  保存 squashed action、evaluate 从 clamp 后 action 反推 raw，导致未更新参数的首 minibatch
  likelihood identity 失败。后续路线改由 `003a-e3a-mjlab-kl-correctness-repair.md` 阻塞；本轮未改
  实现、未跑训练，E3b/E4 仍未启动。
...
状态：**in_progress / 003a_R5_completed_rejected / stopped_before_render_eval**。本节取代 R1 的后续训练路线，但不改写 R1/nominal_v3 的失败证据。
...
#### E3a correctness repair after MJLab/RSL audit

现有 `E3a_adaptive_kl` 保持 immutable rejected diagnostic evidence，不允许用 KL 乘 29 或删除离群
minibatch 后补判通过。只读 audit 发现其 scheduler reduction/timing 与 likelihood identity 不满足
声明的 MJLab/RSL-style 语义，因此在选择 E3b 或进入 E4 前，新增
`003a-e3a-mjlab-kl-correctness-repair.md` 作为 blocking task。

003a 先保存 rollout old distribution params 与 pre-tanh raw action，验证同策略 log-prob identity；
再把 analytic KL 改为 active action 维求和、minibatch 求均值，并在每个 optimizer step 前执行
adaptive decision。环境 action、E2 reward/phase config、batch 2048、minibatch 256、4 epochs、
initial LR `1e-4`、entropy coefficient 与原 E3a gate 均保持不变。no-update correctness gate 通过
之前不得 smoke；未经用户明确授权不得运行新的 2.048M。若以后获准重跑，必须从 E2 config、seed
`72072` 随机初始化写入新的 `E3a_mjlab_kl_repair/`，禁止 resume rejected E3a。repaired gate 通过前，
E3b/E4/20M progression 均保持 blocked。

2026-08-28 执行记录：003a R0-R3 no-update correctness repair 已实现并通过。新增
`verify-e3a-kl-repair` 输出
`artifacts/nominal_v4/unitree_g1/E3a_mjlab_kl_repair/no_update_correctness_gate.json`，SHA
`d2e3650be8e3b112688898bdb8441fb761c14171455a574c1d186adcfbbb80c3`，记录
`training_started=false`、`smoke_started=false`、旧 E3a 仍 rejected、raw/saturated likelihood
identity error `0.0`、same-policy sampled approx KL `0.0`、clip fraction `0.0`、29-active-dim
joint KL fixtures `.004/.01/.021 -> increase/hold/decrease`，真实 `ppo_update` 首 minibatch
telemetry 中 identity error、sampled approx KL、clip fraction、scheduler KL 与 no-update parameter
delta 均为 `0.0`；raw evaluator/mask mismatch 现在 fail closed，并保留原 optimizer KL/clip gate
阈值。验证命令 `py_compile`、`git diff --check` 和 focused pytest 均通过，pytest 结果为
`46 passed`。未经用户明确授权，R4 repaired smoke 与 R5
2.048M rerun 未运行；E3b/E4/20M
progression 继续 blocked。

2026-08-28 执行记录：按用户明确授权只运行 003a R4 repaired smoke，未运行 R5/2.048M。
`E3a_mjlab_kl_repair/smoke/` 完成 4 envs、32 rollout steps、2 updates、256 transitions，
CUDA smoke 正常返回且无 OOM。Smoke artifact SHA：`run_manifest.json`
...
`7722371e68684f521d61beebc681104345e5e20bb01dc1c841a18c303e221c5e` 后续审核发现 fail-open，已拒绝；
smoke 原始 artifacts 未变。正式独立 verifier 从原始 JSON/checkpoint/source 重算 8 个 minibatch、两个
update 的 first-MB identity、243,803 个 policy 参数差分和完整 lineage，生成 schema-3 gate SHA
`27c75fe817d4a8e8f556c37208f248c5e503f4109ae0d2840b3d11652928e693`。Repaired optimizer verifier 已
支持 `E3a_mjlab_kl_repair` 和新的每-minibatch telemetry，并保持原 KL/clip 阈值。

2026-08-30 执行记录：按用户授权在 RTX 5060 Ti、GPU lock、locked/offline `uv run --isolated` 下完成
R5 `32×64×1000=2,048,000` transitions，训练正常返回、无 OOM。`run_manifest.json` SHA
...
`9208a97d7191cec1425504194db776c85c3082902c25efeefacaa5d45d2f1398`。Gate 因 update0 与 fixed
R4/E2 initialization 不完全相等而 fail closed；96,351/243,803 个参数不同，最大差 `1.9372e-7`。
只读重算的 KL mean/p95/max `0.0150795/0.0274182/0.0554737`、clip mean/p95
`0.220988/0.3828125` 也使五项原阈值中的四项失败。`e3a_gate.json` SHA
`65697821095c9f7274d85603b470b074e2ae3393e6b3e58277da215e385294ca`。按路线停止，未运行 render/eval、
E3b 或 E4。
...
### Repaired R4 verifier audit

旧无效 gate SHA `7722371e68684f521d61beebc681104345e5e20bb01dc1c841a18c303e221c5e` 及未满足最终合同的中间 SHA
...
`83f95ccdc5aaaafcbcf50f64d8ccd4c1cfe3ca110b8e3f63fbc97d123aa59a2a` 均保留为 rejected/superseded audit history。
现有 256-transition smoke 由 task-local `task072_e3a_repair_verifier.py repaired-smoke` 重新验证，正式
schema-3 `r4_repaired_smoke_gate.json` SHA 为
`27c75fe817d4a8e8f556c37208f248c5e503f4109ae0d2840b3d11652928e693`，verifier source SHA 为
`f14c173f878006234e09911949ff826b4f6aac3df6d64bb54b1a0b20c9c0192f`。Gate 绑定全部 current sources、
fixed E2/rejected artifacts、ordered reports、scheduler/LR continuity、checkpoint lineage、与 E2 一致的
update0 随机初始化及 policy delta。Locked/offline focused pytest 为 `58 passed, 1 deselected`；
真实 R5 optimizer gate 已失败并停止后续阶段。
