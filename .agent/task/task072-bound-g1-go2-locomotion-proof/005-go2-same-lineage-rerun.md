...
状态：**deferred / standalone_route_superseded_by_Task074_Tier_A**。

本文件保留旧 Task072 G1/Go2 paired-run 合同作为历史审计，不再属于当前 immediate chain。003c G1
walking 与 004 freeze 通过后，先由 Task073 迁移全部 contact-aware assets，再由 Task074 Tier A 对
G1、Go2 和其余 center 全部从随机初始化训练测试；不得只运行本文件后跳过通用资产迁移。
